...
const __getSliderTitle=async (id)=>{
    try{
        const [result] = await connection.execute('select * from slider where id=?', [id])
...
const postSliderData=async ({title, imageDescription, imagePath})=>{
    try{
        await connection.execute('insert into slider(title, image_description, image_path) values(?,?,?)',
