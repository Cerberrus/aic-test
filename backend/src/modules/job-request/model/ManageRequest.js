...
const getJobRequest = async ({ id }) => {
  ...
  const [
    result,
  ] = await connection.execute("select * from job_request where id=?", [id]);
...
const postJobRequest = async ({
  ...
  await connection.execute(
    "insert into job_request(job_vacancy_id, `name`, happy_date, phone_number, sex, email, resume_text, resume_file_path) values(?,?,?,?,?,?,?,?)",
    [
      jobVacancyId,
      name,
      happyDate,
...
const updateJobRequest = async (
  ...
  jobVacancyId = Number(jobVacancyId);
  await connection.execute(
    "update job_request set job_vacancy_id=?, name=?, happy_date=?, phone_number=?, sex=?, email=?, resume_text=?, resume_file_path=? where id =?",
    [
      jobVacancyId,
      name,
      happyDate,
